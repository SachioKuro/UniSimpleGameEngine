...
namespace Core {
	namespace Terrain {
		WorldTree::WorldTree(Camera* camera) : camera(camera) {
			...
			noise = new PerlinNoise(1337 + 10, NOISE_WIDTH, NOISE_HEIGHT, CHUNK_SIZE_X, CHUNK_SIZE_Z, 34, 30000);
			bnoise = new PerlinNoise(1337 - 10, NOISE_WIDTH, NOISE_HEIGHT, CHUNK_SIZE_X, CHUNK_SIZE_Z, 3, 80000);
								...
								if (currentChunk->getCenter().x + chunkSizeHalfX < cpos.x) {
									...
									for (int l = 0; l < WORLDSIZE; l++) {
										...
										chunks[WORLDSIZE - 1][l] =
											new Chunk(ivec3(chunks[WORLDSIZE - 2][l]->getCenter().x + chunkSizeHalfX, 0, chunks[WORLDSIZE - 2][l]->getCenter().z + chunkSizeHalfZ),
									...
									for (int l = 0; l < WORLDSIZE; l++) {
										...
										chunks[0][l] =
											new Chunk(ivec3(chunks[1][l]->getCenter().x - CHUNK_SIZE_X - chunkSizeHalfX, 0, chunks[1][l]->getCenter().z + chunkSizeHalfZ),
									...
									for (int k = 0; k < WORLDSIZE; k++) {
										...
										chunks[k][0] =
											new Chunk(ivec3(chunks[k][1]->getCenter().x - chunkSizeHalfX, 0, chunks[k][1]->getCenter().z + CHUNK_SIZE_Z + chunkSizeHalfZ),
												*(noise->createNoise(noisePos.x, noisePos.y)), 
												*(bnoise->createNoise(noisePos.x, noisePos.y)),
									...
									for (int k = 0; k < WORLDSIZE; k++) {
										...
										chunks[k][WORLDSIZE - 1] =
											new Chunk( ivec3(chunks[k][WORLDSIZE - 2]->getCenter().x - chunkSizeHalfX, 0, chunks[k][WORLDSIZE - 2]->getCenter().z - chunkSizeHalfZ),
